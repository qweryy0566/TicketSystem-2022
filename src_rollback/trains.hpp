#ifndef TICKETSYSTEM_ROLLBACK_TRAINS_HPP_
#define TICKETSYSTEM_ROLLBACK_TRAINS_HPP_
...
class TrainManagement {
 ...
 public:
  TrainManagement()
      : trains{"trains_index", "trains"},
        train_dates{"train_dates_index", "train_dates"},
        station_trains{"station_trains_index", "station_trains"},
        ticket_trains{"ticket_trains_index", "ticket_trains"},
        orders{"orders_index", "orders"},
        pending_orders{"pending_index", "pending"} {}
  bool AddTrain(TokenScanner &token, const int &timestamp) {
    ...
    trains.Insert(timestamp, trainid, 0, new_train);
    return 1;
  }
  ...
  bool DeleteTrain(const string &train_id, const int &timestamp) {
    ...
    return trains.Delete(timestamp, trainid, 0);
  }
  ...
  bool ReleaseTrain(const string &train_id, const int &timestamp) {
    ...
    trains.Modify(timestamp, trainid, 0, the_train);
    train_dates.Insert(timestamp, trainid, 0,
    ...
    for (int i = 0; i < the_train.station_num; ++i) {
      station_trains.Insert(
          timestamp, StationHash(the_train.stations[i]), trainid,
  ...
  string BuyTicket(const string &username, const string &train_id,
    ...
    if (left >= cnt) {
      ...
      if (is_buy)
        ticket_trains.Modify(timestamp, trainid, dept_date, ticket);
      else
        ticket_trains.Insert(timestamp, trainid, dept_date, ticket);
      ...
      pending_orders.Insert(timestamp, {trainid, dept_date}, timestamp,
                            pending_info);
      ret = "queue";
    }
    // 注意：候补也要加入订单记录内。
    orders.Insert(timestamp, userid, -timestamp, the_order);
    return ret;
  }
  ...
  bool RefundTicket(const string &username, const unsigned &rank,
    ...
    orders.Modify(timestamp, userid, -the_order.timestamp, the_order);
    if (old_status == Order::PENDING) {
      pending_orders.Delete(timestamp, {trainid, dept_date},
                            the_order.timestamp);
    } else {
      TicketTrain ticket{ticket_trains.Get(trainid, dept_date)};
      // 把票买回去。
      ...
      for (auto it : pendings)
        if (ticket.QueryTicket(it.s_order, it.t_order - 1) >= it.need) {
          ...
          orders.Modify(timestamp, it.userid, -it.timestamp, the_order);
          pending_orders.Delete(timestamp, {trainid, dept_date}, it.timestamp);
        }
      ticket_trains.Modify(timestamp, trainid, dept_date, ticket);
    }
    return 1;
...
#endif  // TICKETSYSTEM_ROLLBACK_TRAINS_HPP_