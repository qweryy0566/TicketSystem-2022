...
- `lib/` 中含有一些需要用到的数据结构和工具类、函数的代码，如 B+ 树、vector、linked_hashmap 等，`utils.hpp` 里主要含有快速排序、字符串解析类、定长字符串类、哈希函数类、时间日期类等~~，`test_logic.hpp` 是用 `std::map` 实现的假内存 B+ 树，仅用于测试逻辑主体~~。
- `src/` 中为主体逻辑代码：
