...
- `lib/` 中含有一些需要用到的数据结构和工具类、函数的代码：
  -  B+ 树、vector 与 linked_hashmap（为方便与 STL 互通，在代码中的类名为 unordered_map）。
  -  `exceptions.hpp` 为抛出异常类实现文件，用于显示异常信息。
  -  `utils.hpp` 里主要含有快速排序、字符串解析类、定长字符串类、哈希函数类、时间日期类等。
  -  `test_logic.hpp` 是用 `std::map` 实现的假内存 B+ 树，仅用于测试逻辑主体。
- `src/` 中为主体逻辑代码：
  - `main.cpp` 为主入口，将输入的指令传给命令解析类，并负责将返回的信息输出。
  - `commands.hpp` 负责解析命令，调用对应的执行函数。
  - `users.hpp` 里含有用户管理类，执行和用户相关的操作。
  - `trains.hpp` 里含有火车管理类，执行和火车、车票相关的操作，包括但不限于添加/删除车次、查票、买票、管理订单等。
