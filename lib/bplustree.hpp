...
template<typename _key_type, typename _subkey_type, typename _value_type, int M>
struct Node{
    long long int valid_pos=0;
    int node_size=0;
    Lead<_key_type, _subkey_type, _value_type> array_lead[M+1];
    int tag=0;//if tag==1, it points to leaves(block) 
};

template<typename _key_type, typename _subkey_type, typename _value_type, int L>
struct Block{
    long long int valid_pos=0;
    int block_size=0;
    Element<_key_type, _subkey_type, _value_type> array_element[L+1];
...
class BPlusTree{
    ...
    unordered_map<pair<_key_type, _subkey_type>, _value_type, PairHash> cache;
public:
    ...
    long long int GetValidPosition(int tag){//tag==1 return for a node, else for a block
        ...
        if(tag){
            ...
            if(!valid_pos){
                ans=2*sizeof(int)+2*sizeof(long long int)+(long long)count_node*sizeof(Node<_key_type, _subkey_type, _value_type, M>);
            }
            else{
                ans=valid_pos;
                ...
                file_node_.write(reinterpret_cast<char*>(&test_node),sizeof(Node<_key_type, _subkey_type, _value_type, M>));
            }
            count_node++;
            ...
            if(!valid_pos){
                ans=sizeof(int)+sizeof(long long int)+(long long)count_block*sizeof(Block<_key_type, _subkey_type, _value_type, L>);
            }
            else{
                ans=valid_pos;
                ...
                file_block_.write(reinterpret_cast<char*>(&test_block),sizeof(Block<_key_type, _subkey_type, _value_type, L>));
            }
            count_block++;
            file_block_.seekp(0);
    ...
    bool Insert(const _key_type& key, const _subkey_type& subkey, const _value_type& value) {
        ...
        file_node_.read(reinterpret_cast<char *>(&count_all),sizeof(int));
        if(!count_all){
            Block<_key_type, _subkey_type, _value_type, L> new_block;
            ...
            file_block_.write(reinterpret_cast<char*>(&new_block),sizeof(Block<_key_type, _subkey_type, _value_type, L>));
            new_lead.son_pos=pos_new_block;
            Node<_key_type, _subkey_type, _value_type, M> new_node;
            ...
            file_node_.write(reinterpret_cast<char*>(&new_node),sizeof(Node<_key_type, _subkey_type, _value_type, M>));
            count_all=1;
            file_node_.seekp(0);
            file_node_.write(reinterpret_cast<char*>(&count_all),sizeof(int));
            file_node_.seekp(sizeof(int));
            ...
            if(ind!=-1&&new_element==test_block.array_element[ind]){//new one has been in the block
                ...
                if(ind==-1){//all elements in the block < the targeted
                    ...
                    if(!ind){//lead must be updated
                        Node<_key_type, _subkey_type, _value_type, M> update_node;
                        for(int i=top;i>=0;i--){
                            file_node_.seekg(stack_pos[i]);
                            file_node_.read(reinterpret_cast<char*>(&update_node),sizeof(Node<_key_type, _subkey_type, _value_type, M>));
                            ...
                            file_node_.write(reinterpret_cast<char*>(&update_node),sizeof(Node<_key_type, _subkey_type, _value_type, M>));
                ...
                if(test_block.next){
                    Block<_key_type, _subkey_type, _value_type, L> next_block;
                    file_block_.seekg(test_block.next);
                    file_block_.read(reinterpret_cast<char*>(&next_block),sizeof(Block<_key_type, _subkey_type, _value_type, L>));
                    next_block.last=new_block_pos;
                    file_block_.seekp(test_block.next);
                    file_block_.write(reinterpret_cast<char*>(&next_block),sizeof(Block<_key_type, _subkey_type, _value_type, L>));
                }
    ...
    bool Delete(const _key_type& key, const _subkey_type& subkey) {
        ...
        if (test_block.block_size >= L >> 1) {
            file_block_.seekp(pos);
            file_block_.write(reinterpret_cast<char *>(&test_block), sizeof(Block<_key_type, _subkey_type, _value_type, L>));
        }
        else{//test_block.block_size < L >> 1
            Block<_key_type, _subkey_type, _value_type, L> adjacent_block;
            if(top==0){//merge happens in the root node
                if(stack_next_pos[top+1]!=-1){
                    file_block_.seekg(test_block.next);
                    file_block_.read(reinterpret_cast<char*>(&adjacent_block),sizeof(Block<_key_type, _subkey_type, _value_type, L>));
                    if(adjacent_block.block_size > L >> 1){//borrow its son
                        ...
                        file_block_.seekp(test_block.next);
                        file_block_.write(reinterpret_cast<char*>(&adjacent_block),sizeof(Block<_key_type, _subkey_type, _value_type, L>));
                        Node<_key_type, _subkey_type, _value_type, M> update_node;
                        file_node_.seekg(stack_pos[top]);
                        file_node_.read(reinterpret_cast<char*>(&update_node),sizeof(Node<_key_type, _subkey_type, _value_type, M>));
                ...
                if(check_){
                    Node<_key_type, _subkey_type, _value_type, M> update_node;
                    Node<_key_type, _subkey_type, _value_type, M> adjacent_node;
                    while(top>=1){
                        file_node_.seekg(stack_pos[top]);
                        file_node_.read(reinterpret_cast<char*>(&update_node),sizeof(Node<_key_type, _subkey_type, _value_type, M>));
                        ...
                        if(stack_next_pos[top]!=-1){//has next_node
                            ...
                            file_node_.read(reinterpret_cast<char*>(&adjacent_node),sizeof(Node<_key_type, _subkey_type, _value_type, M>));
                            if(adjacent_node.node_size > M>>1){//borrow its son
                                ...
                                file_node_.write(reinterpret_cast<char*>(&adjacent_node),sizeof(Node<_key_type, _subkey_type, _value_type, M>));
                                file_node_.seekp(stack_pos[top]);
                                file_node_.write(reinterpret_cast<char*>(&update_node),sizeof(Node<_key_type, _subkey_type, _value_type, M>));
